...
class Auth {
    ...
    */
    public static function logout(){
        self::unsetUsuario(); //Destroi usuário da sessão
        session_unset(); //Limpa outras variáveis da sessão
    }      
    ...
    public static function checkAuth($redirect=''){  
        ...
        if (is_object($objUsuario)) {
            ...
            \Auth::setMessage(Error::eException("SESSION_EXPIRED"));
    ...
    public static function checarPermissao($arrUserPerfil, $redirect='/app/login'){    
        ...
        if(array_search($user->CODIGO_PERFIL, $arrUserPerfil) === false){
            ...
            \Auth::setMessage(Error::eException("SESSION_PERMISSION"));
            header('Location: ' . $redirect);
            die();
        }
    ...
    public static function persistUsuario($objUsuario){
        if ($objUsuario instanceof common\classes\helpers\Usuario || $objUsuario instanceof UsuarioTeste) $_SESSION[self::SESSION_USER] = serialize($objUsuario);
    }
    ...
    public static function getUsuario(){
        ...
        if (isset($_SESSION[self::SESSION_USER])) {
            $objUsuarioRecup = unserialize($_SESSION[self::SESSION_USER]);
             if ($objUsuarioRecup instanceof common\classes\helpers\Usuario || $objUsuarioRecup instanceof UsuarioTeste) {
